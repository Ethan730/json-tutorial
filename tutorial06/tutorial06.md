...
之前在lept_parse_string中，解析成功后设置字符串是通过`lept_set_string(v, (const char*)lept_context_pop(c, len), len)`。可是现在多了一层调用，还要使用之前的`lept_context_pop`函数，就需要把这个函数返回的指针传递到上一层，因此需要采用指针传参的方式，同时由于本身为指针，因此形参就变成了`char**`。
